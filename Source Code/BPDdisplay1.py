...
class ClickWidget(QLabel):
    ...
    def mousePressEvent(self, event):
        ...
        if self.main.mode == 2:
            self.main.BPD.__init__(self.main.BPD.size, self.main.BPD.elbows)
            crosspipes = []
...
class Window(QMainWindow):
    def __init__(self,n=default_size,BPD=[]):
        ...
        if BPD == []:
            ...
            self.perm = BPD.perm
            self.BPD = BPD
        self.setGeometry(QRect(0, 0, 96*(self.n)+150, 96*(self.n)+150))
    ...
    def Dream(self, BPD):
        BPD = BPDclass.BPD(BPD.size,BPD.elbows)
        if BPD.size != self.n:
            self.close()
        ...
        for i in range(self.n):
            ...
            for j in range(self.n):
                self.pixmaps[i].append(QPixmap(96, 96))
                self.pixmaps[i][j].fill(Qt.transparent)
                painter = QPainter(self.pixmaps[i][j])
                painter.drawPixmap(0,0,96,96,QPixmap('images/empty.png'))
                if [j+1,i+1] in BPD.elbows:
                    painter.drawPixmap(0,0,96,96,QPixmap('images/elbow.png'))
                painter.end()
                ...
                if place in BPD.elbows:
                    if direction == 1:
                        if pipe not in BPD.badpipes:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/se'+colors[self.color[pnum]]+'.png'))
                        else:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/seBAD.png'))
                    else:
                        if pipe not in BPD.badpipes:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/nw'+colors[self.color[pnum]]+'.png'))
                        else:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/nwBAD.png'))
                    direction = -1*direction
                    
                else:
                    if direction == 1:
                        if pipe not in BPD.badpipes:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/vert'+colors[self.color[pnum]]+'.png'))
                        else:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/vertBAD.png'))
                    else:
                        if pipe not in BPD.badpipes:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/hort'+colors[self.color[pnum]]+'.png'))
                        else:
                            painter.drawPixmap(0,0,96,96,QPixmap('images/hortBAD.png'))
                painter.end()
    ...
    def keyPressEvent(self, event):
        ...
        #This is an Easter egg. Email me if you want to see the easter egg (especially if you have ever played Pyuo Pyuo Tetris 2)
        elif event.key() == Qt.Key_L:
            Lidellenum = random.randint(1,12)
            Lidelletext = 'Lidelle/Lidelle'+str(Lidellenum)+'.png'
            oImage = QImage(Lidelletext)
            sImage = oImage.scaled(QSize(96*(self.n)+150, 96*(self.n)+150))
            palette = QPalette()
    ...
    def setupUI(self, Interface):
        ...
        self.scrollAreaWidgetContents.setGeometry(QRect(0, 0, 96*(self.n)+100, 96*(self.n)+100))
        ...
        for j in range(self.n):
            ...
            self.labels[self.n][j].setFont(QFont('Arial', 20))
        ...
        for i in range(self.n):
            ...
            self.labels[i][self.n].setFont(QFont('Arial', 20))
