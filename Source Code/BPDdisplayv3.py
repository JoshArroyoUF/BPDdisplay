...
highlight = False
show_bad = True
have_one = ast.literal_eval(parser.get("config", "have_one"))
have_all = ast.literal_eval(parser.get("config", "have_all"))
avoid_all = ast.literal_eval(parser.get("config", "avoid_all"))
marked = []
mark_droops_for_pat = ast.literal_eval(parser.get("config", "mark_droops_for_pat"))
equal_rate = ast.literal_eval(parser.get("config", "equal_rate"))
marker = ast.literal_eval(parser.get("config", "marker"))
...
class ClickWidget(QLabel):
    ...
    def mousePressEvent(self, event):
        ...
        if event.buttons() == Qt.RightButton:
            ...
            mark_flag = False
            for m in range(len(marked)-1,-1,-1):
                if marked[m][0] == [self.row, self.col]:
                    mark_flag = True
                    global marker
                    marker = marked[m][1]
                    marked.pop(m)
            ...
            if not mark_flag:
                marked.append([[self.row, self.col], marker])
...
class Window(QMainWindow):
    ...
    def Dream(self, BPD):
        ...
        for loc in marked:
            painter = QPainter(self.pixmaps[loc[0][1]-1][loc[0][0]-1])
            painter.drawPixmap(0,0,scale,scale,QPixmap('images/marked'+str(loc[1])+'.png'))
            painter.end()
        ...
        for pipe in BPD.pipes:
            ...
            for loc in marked:
                self.labels[loc[0][1]-1][loc[0][0]-1].setPixmap(self.pixmaps[loc[0][1]-1][loc[0][0]-1])
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_I:
            ...
            goodflag1 = True
            goodflag2 = True
            while badflag or goodflag1 or goodflag2:
                badflag = False
                ...
                if not badflag:
                    ...
                    if equal_rate:
                        have_one = [random.choice(have_one)]
                    
                    for pat in have_one:
                        pat_test = BPDclass.permpatterncontainment(testperm, pat, True)
                        if pat_test != False:
                            goodflag1 = False
                            
                            if len(have_all) == 0:
                                randpat = random.randint(0,len(colors)-1)
                                ...
                                for i in range(len(self.color)):
                                    if i+1 in pat_test:
                                        self.color[i] = randpat
                                    else:
                                        self.color[i] = randc
                                        
                                
                                if mark_droops_for_pat:
                                    global marked
                                    marked = []
                                    if pat == [1,4,2,3]:
                                        marked.append([[testperm.index(pat_test[0])+1, pat_test[0]],1])
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[3]],1])
                                        
                                    if pat == [1,2,5,4,3]:
                                        marked.append([[testperm.index(pat_test[0])+1, pat_test[0]],2])
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[1]],1])
                                        
                                        marked.append([[testperm.index(pat_test[2])+1, pat_test[3]],1])
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[4]],2])
                                        
                                    if pat == [1,3,2,5,4]:
                                        marked.append([[testperm.index(pat_test[0])+1, pat_test[0]],2])
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[1]],1])
                                        
                                        marked.append([[testperm.index(pat_test[3])+1, pat_test[4]],1])
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[1]],2])
                                        
                                    if pat == [2,5,1,4,3]:
                                        marked.append([[testperm.index(pat_test[0])+1, pat_test[0]],1])
                                        marked.append([[testperm.index(pat_test[2])+1, pat_test[2]],2])
                                        
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[3]],1])
                                        marked.append([[testperm.index(pat_test[3])+1, pat_test[4]],2])
                                    ...
                                    if pat == [2,1,5,6,4,3]:
                                        ...
                                        marked.append([[testperm.index(pat_test[3])+1, pat_test[4]],1])
                                        marked.append([[testperm.index(pat_test[2])+1, pat_test[5]],2])
                                    ...
                                    if pat == [2,4,1,6,5,3]:
                                        marked.append([[testperm.index(pat_test[0])+1, pat_test[0]],3])
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[1]],1])
                                        marked.append([[testperm.index(pat_test[2])+1, pat_test[2]],2])
                                        
                                        marked.append([[testperm.index(pat_test[3])+1, pat_test[4]],1])
                                        marked.append([[testperm.index(pat_test[4])+1, pat_test[5]],2])
                                        marked.append([[testperm.index(pat_test[1])+1, pat_test[1]],3])
            ...
            to_print = BPDclass.BPD(self.BPD.size,self.BPD.elbows,self.co)
            if self.co:
                to_print.coBPD()
